...
class ImageCreator:
    ...
    def create_image(self, idx, kind, kind_idx, color_bg='random', color_text='random'):
        ...
        if font_weight > 0 and font_weight <= 300:
            font_weight = 'Thin'
        elif font_weight > 300 and font_weight <=450:
            font_weight = 'Reqular'
        elif font_weight > 450 and font_weight <=650:
            font_weight = 'Medium'
        elif font_weight > 650 and font_weight <=1000:
            font_weight = 'Bold'
        ...
        self.data[image_name] = {
            '폰트종류': kind,
            '폰트굵기': font_weight,
            '폰트색상': [text_color[0],text_color[1],text_color[2]]
        }
...
generate_font = ['고딕', '명조', '장식체', '고전체', '손글씨']

idx = 0
for j in generate_font:
    for i in range (600):
        
        ImageCreator().create_image(i, j, idx, color_text='random', color_bg='random')
        ImageCreator().create_image(i, j, idx, color_bg='black', color_text='white')
        ImageCreator().create_image(i, j, idx, color_bg='white', color_text='black')
        print(f'{i} complete, index = {idx}')
    idx = idx+1